 ...
 *
 * Why we just don't use the TS parser from TYPO3? Because CMP§ should work standalone too.
 *
 * @author	Kasper Skaarhoj <kasperYYYY@typo3.com>
 * @subpackage   Config
 * @package    CMP3
...
class TypoScriptParser {
	...
	public static function ParseAsArray($tsConfString)
	{
		$parseObj = new self;
		$parseObj->parse($tsConfString);

		return $parseObj->setup;
	}


	public static function ParseFileAsArray($filepath)
	{
		if (is_object(\tx_cmp3::$System)) {
			$filepath = \tx_cmp3::$System->ResolvePath($filepath);
		} else {
			$filepath = \tx_cmp3::ResolvePath($filepath);
		}
		$tsConfString = file_get_contents($filepath);

		return self::ParseAsArray($tsConfString);
	}
							...
							if ($this->strict && preg_match('/[^[:alnum:]_\.-]/i',$objStrName,$r))	{
								...
								if (preg_match('/^:=([^\(]+)\((.+)\).*/', $line, $match))	{
									...
									switch ($tsFunc)	{
										...
										break;
										case 'removeFromList':
											$existingElements = $this->trimExplode(',',$currentValue);
											$removeElements = $this->trimExplode(',',$tsFuncArg);
										...
										default:
											if (isset($TYPO3_CONF_VARS['SC_OPTIONS']['t3lib/class.t3lib_tsparser.php']['preParseFunc'][$tsFunc]))	{
												...
												$fakeThis = FALSE;
												$newValue = \t3lib_div::callUserFunction($hookMethod,$params,$fakeThis);
											} else {
												throw new Exception('Missing function definition for '.$tsFunc.' on TypoScript line '.$lineP,'Core',2);
											}
									}
	...
	function checkIncludeLines($string, $cycle_counter=1)	{
		if ($cycle_counter>100) {
			throw new Exception('It appears like TypoScript code is looping over itself. Check your templates for "&lt;INCLUDE_TYPOSCRIPT: ..." tags','Core',2);
			return '';
		}
			...
			while(list($c,$v)=each($allParts))	{
				if (!$c)	{	 // first goes through
					...
					if (preg_match('/^\s*\r?\n/',$subparts[1]))	{	// There must be a line-break char after
						...
						$params = $this->get_tag_attributes($subparts[0]);
						if ($params['source'])	{
							$sourceParts = explode(':',$params['source'],2);
							switch(strtolower(trim($sourceParts[0])))	{
								case 'file':
									$filename = \tx_cmp3::$System->ResolvePath(trim($sourceParts[1]));
									if (strcmp($filename,''))	{	// Must exist and must not contain '..' and must be relative
										if (@is_file($filename) && filesize($filename)<100000)	{	// Max. 100 KB include files!
											...
											$included_text = self::checkIncludeLines(file_get_contents($filename),$cycle_counter+1);
