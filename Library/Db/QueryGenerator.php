...
#TODO remove Zend_Db_Expr?

/**
 * Class for SQL SELECT generation and results.
 *
 * This is a modified version of Zend_Db. The modifications are not that big so syncing with Zend should be possible.
 *
 *
 * @author     adopted by Rene Fritz (r.fritz@bitmotion.de)
 * @subpackage Db
 * @package    CMP3
 * @copyright  Copyright (c) 2005-2008 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
...
class QueryGenerator
{
    ...
    {
        $this->_adapter = is_null($adapter) ? new \Cmp3\Db\QueryGeneratorT3 : $adapter;
     ...
     * @return Zend_Db_Select This Zend_Db_Select object.
     */
    public function where($cond, $value = null, $type = null, $key = null)
    {
    	if ($cond) {
    		$key = $key ? $key : uniqid();
        	$this->_parts[self::WHERE][$key] = $this->_where($cond, $value, $type, true);
    	}
     ...
     * @see where()
     */
    public function orWhere($cond, $value = null, $type = null, $key = null)
    {
    	$key = $key ? $key : uniqid();
        $this->_parts[self::WHERE][$key] = $this->_where($cond, $value, $type, false);
...
#TODO Zend_Db_Expr
    ...
    {
        if ($count!==null) $this->_parts[self::LIMIT_COUNT]  = (int) $count;
        if ($offset!==null) $this->_parts[self::LIMIT_OFFSET] = (int) $offset;
        return $this;
    }
...
#TODO Zend_Db_Expr
        } else if ($name instanceof Zend_Db_Expr || $name instanceof Zend_Db_Select || $name instanceof \Cmp3\Db\QueryGenerator) {
            $tableName = $name;
...
#TODO Zend_Db_Expr
    ...
    protected function _renderWhere($sql)
    {
        if ($this->_parts[self::FROM] && $this->_parts[self::WHERE]) {
            $sql .= ($sql ? ' ' . self::SQL_WHERE . ' ' : '') .  implode(' ', $this->_parts[self::WHERE]);
        }
    ...
    protected function _renderGroup($sql)
    {
        if ($this->_parts[self::FROM] && $this->_parts[self::GROUP]) {
            ...
            $sql .= ($sql ? ' ' . self::SQL_GROUP_BY . ' ' : '') . implode(",\n\t", $group);
        }
    ...
    protected function _renderOrder($sql)
    {
        if ($this->_parts[self::ORDER]) {
            ...
            $sql .= ($sql ? ' ' . self::SQL_ORDER_BY . ' ' : '') . implode(', ', $order);
        }
        ...
        foreach (array_keys(self::$_partsInit) as $part) {
            ...
            if (method_exists($this, $method)) {
                ...
                switch ($part) {
					...
					break;
					case self::GROUP:
						$queryParts['GROUPBY'] .= $sql;
					break;
					case self::HAVING:
						$queryParts['GROUPBY'] .= $sql;
					break;
					case self::ORDER:
						$queryParts['ORDERBY'] .= $sql;
					break;
					case self::LIMIT_COUNT:
						$queryParts['LIMIT'] .= $sql;
					break;
