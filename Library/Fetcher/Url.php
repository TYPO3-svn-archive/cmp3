 ...
 * @package    CMP3
 */
class Url extends FetcherAbstract {
	...
	{
		$objFetch = new \Cmp3\Files\Fetch($this->strUrl);
		$strContent = $objFetch->GetContent();
		if ($objFetch->hasError()) {
			throw new \Cmp3\Files\Exception($objFetch->GetErrorMessage());
		}

		if (!$this->strContentType) {
			$path_parts = self::_split_fileref($this->strUrl);
			$this->strContentType = self::GetContentTypeForSuffix($path_parts['realFileext']);
		}
