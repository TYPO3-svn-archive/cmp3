    ...
    public function transformToURI($xml, $uri)
    {
        ...
        if (preg_match('/CLI$/', $this->interface)) {
            ...
            if ($this->processor === 'SAXON') {
                ...
                $this->command = ($isJAVACLI ? escapeshellarg($this->runtime) . ' -cp ' : null) . escapeshellarg(($isJAVACLI ? dirname(__FILE__) . DIRECTORY_SEPARATOR . 'XSLT2Processor.jar' . PATH_SEPARATOR : null) . $this->processorPath) . ($isJAVACLI ? ' net.sf.saxon.Transform' : null) . $commandOptions . ' -o' . ($this->processorVersion != 8 ? ':' : ' ') . $inputs['output'] . ($this->processorVersion != 8 ? ' -s:' : ' ') . $inputs['source'] . ($this->stylesheet !== XML_XSLT2PROCESSOR_PI ? ($this->processorVersion != 8 ? ' -xsl:' : ' ') . $inputs['stylesheet'] : null) . ' ' . $params;
